...
class KaraokeUnit
{
    ...
    static Parse(text)
    {
        ...
        {
            const ttelements = ("[00:00.00]" + text).match(/\[\d+:\d+[:.]\d+\].*?((?=\[\d+:\d+[:.]\d+\])|$)/gm);
            ttelements.forEach(tte => {elements.push(TimeTagElement.Create(tte));});
    ...
    ComplementAllTime(widths)
    {
        if (this.text_array.length !== widths.length || this.start_time < 0 || this.end_time < 0)
            return false;
        for (let i = 0;i < this.text_array.length - 1;i++)
        {
            ...
            const pt = this.start_times[i];
            const pc = widths[i];
            let nc = 0;
            let nt = 0;
            for (let j = i+1;j < this.text_array.length;j++)
            {
                nc += widths[j];
                if (this.end_times[j] >= 0)
                {
                    nt = this.end_times[j];
                    break;
                }
            }
            const divtime = (pt * nc + nt * pc) / (pc + nc);
            this.end_times[i] = this.start_times[i+1] = divtime;
        }
    }
    ...
    Complement()
    {
        ...
        for (let i = 0;i < this.units.length - 1;i++)
        {
            if (this.units[i].end_time < 0 && this.units[i+1].start_time < 0)
            {
                ...
                const divtime = (prev.time * nc + nt * prev.count) / (prev.count + nc);
